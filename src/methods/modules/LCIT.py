...
def MLP(in_features: int, out_features: int, hidden_sizes: None, norm=True, activator=nn.Tanh, activator_params=None):
...
class UniformFlow(nn.Module):
    def __init__(self, dz, n_components, hidden_sizes, activator=nn.Tanh, activator_params=None):
        super().__init__()
        activator_params = activator_params or {}
    ...
    def forward(self, X, Z):
        ...
        p_hat = torch.clip(p_hat, min=1e-8, max=None)
        log_de_dx = p_hat.log()
...
class DualCNF(LightningModule):
    def __init__(self, dz, n_components, hidden_sizes, lr, l1, l2, verbose):
        ...
        self.x_cnf = UniformFlow(dz=dz, n_components=n_components, hidden_sizes=hidden_sizes)
        self.y_cnf = UniformFlow(dz=dz, n_components=n_components, hidden_sizes=hidden_sizes)
    ...
    def loss(self, X, Y, Z):
        ...
        l1 = sum(torch.linalg.norm(p, 1) for p in self.parameters() if p.requires_grad_ and torch.numel(p) > 0)
        # c = sum(torch.numel(p) for p in self.parameters() if p.requires_grad_)
    ...
    def fit(self, X, Y, Z, max_epochs, verbose, gpus=None, callbacks=None):
        ...
        with warnings.catch_warnings():
            ...
            val_dataloader = DataLoader(valid, batch_size=256)
...
def LCIT(X, Y, Z, normalize=True, n_components=32, hidden_sizes=4, lr=5e-3, l1=5e-4, l2=5e-5, max_epochs=100, random_state=0, gpus=0, return_latents=False, verbose=False, **kwargs):
    ...
    if normalize:
        ...
        X, Y, Z = map(lambda x: strip_outliers(x, 0.025), (X, Y, Z))
    X, Y, Z = map(lambda x: torch.tensor(x, dtype=torch.float32).view(N, -1), (X, Y, Z))

    model = DualCNF(dz, n_components=n_components, hidden_sizes=hidden_sizes, lr=lr, l1=l1, l2=l2, verbose=verbose)
