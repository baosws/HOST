...
from experiments.real.viz import viz_perm
...
if __name__ == '__main__':
    ...
    parser.add_argument('--methods', type=str, nargs='+', default=['HOST'])
    ...
    for dataset in data_config['datasets']:
        X = pd.read_csv(f'data/{dataset}.csv').values
        GT = pd.read_csv(f'data/{dataset}_dag.csv', index_col='source').values
        for i in range(data_config['T']):
